...
#SBATCH --output=fairseq_train_no_attention_%J.out
#SBATCH --error=fairseq_train_no_attention_%J.err
#SBATCH --job-name=fairseq_train_no_attention
...
fairseq-train data-bin/writingPrompts --save-dir ./no_attention_checkpoints -a fconv_self_att_wp --lr 0.25 --optimizer nag --clip-norm 0.1 --max-tokens 1500 --lr-scheduler reduce_lr_on_plateau --decoder-attention True --encoder-attention False --criterion label_smoothed_cross_entropy --weight-decay .0000001 --label-smoothing 0 --source-lang wp_source --target-lang wp_target --gated-attention False --self-attention False --project-input True --skip-invalid-size-inputs-valid-test --pretrained False --no-epoch-checkpoints
